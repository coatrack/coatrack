...
/**
 * Provides a local cache for API keys, allowing the gateway to validate API keys without connection to CoatRack admin
 * application. Caches (and periodically refreshes) a list of all API keys assigned to all services offered by the
 * gateway.
 *
...
public class LocalApiKeyManager {
    ...
    public LocalApiKeyManager(ApiKeyFetcher apiKeyFetcher,
    ...
    public ApiKey getApiKeyEntityFromLocalCache(String apiKeyValue) {
        ...
        if (!isLocalApiKeyListInitialized) {
            ...
            throw new OfflineWorkingTimeExceedingException(
                    "The predefined time for working in offline mode is exceeded. The "
    ...
    private ApiKey extractApiKeyFromLocalApiKeyList(String apiKeyValue) {
        ...
        Optional<ApiKey> optionalApiKey = localApiKeyList.stream()
                .filter(apiKeyFromLocalList -> apiKeyFromLocalList.getKeyValue().equals(apiKeyValue)).findFirst();

        if (optionalApiKey.isPresent()) {
            return optionalApiKey.get();
        } else {
            throw new ApiKeyNotFoundInLocalApiKeyListException(
                    "The API key with the value " + apiKeyValue + " could not be found in the local API key list.");
        }
    }
    ...
    private void updateApiKeyList(List<ApiKey> fetchedApiKeyList) {
        Assert.notNull(fetchedApiKeyList,
                "The local API key list will not be " + "updated since the fetched API key list was null.");
    ...
    private void logGatewayMode(GatewayMode currentGatewayMode) {
        log.info(currentGatewayMode == GatewayMode.ONLINE ? switchingToOnlineModeMessage
                : switchingToOfflineModeMessage);
    }

    /*
     * If the gateway successfully receives the latest list of API keys from CoatRack admin, it goes to online mode. If
     * a connection attempt to CoatRack admin server failed, it goes to the time-limited functioning offline mode.
     */
