...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                ...
                .antMatchers(PROXY_CONFIG_URI_PATTERN).hasRole("USER")
                // admin is allowed to access admin config and credentials
                .antMatchers(ADMIN_CONFIG_URI_PATTERN, CREDENTIALS_URI_PATTERN).hasRole("ADMIN")
                // add a "catchall" rule to assure that no URLs are accessible without authentication (e.g. "/" not
                // covered by above patterns)
                .anyRequest().hasRole("ADMIN").and().httpBasic().authenticationEntryPoint(authenticationEntryPoint)
                .and().sessionManagement()
                // do not create sessions, because proxies/admin should be authenticated on each request
                .sessionCreationPolicy(SessionCreationPolicy.STATELESS);
        http.addFilterAfter(filter, BasicAuthenticationFilter.class).csrf().disable();
