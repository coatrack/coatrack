...
public class CheckIfUrlAndCredentialsFitWithEachOtherFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (request instanceof HttpServletRequest
                // added the following check to avoid "IllegalStateException: resource already
                // commited"
            ...
            // Accepted URI - only check in case it is a proxy URI, other cases are covered
            ...
            if (url.startsWith(PROXY_URI)) { // || url.startsWith(ADMIN_URI) || url.startsWith(CREDENTIAL_URI)) {
                ...
                if (authorization != null && authorization.toLowerCase().startsWith("basic")) {
                    ...
                    if (values != null && values.length > 0) {
                        ...
                        // check if the proxy is accessing only URLs that this proxy is allowed to
                        // access
                        ...
                        // Retrieve the Id in Long format TODO Consider to separate the business key to
                        // the technical key which generates such kind of dirty logic
                        /**
                         * For the Proxy user case
                         */
                        // It is a proxy credential key, then retrieve the credential
                        /*
                         * ConfigServerCredential credential =
                         * configServerCredentialRepository.findOneByName(user); if
                         * (url.startsWith(PROXY_URI) && credential == null) { // There is no
                         * credential, then returns an error
                         * httpResponse.sendError(HttpServletResponse.SC_UNAUTHORIZED,
                         * "Credenctial is missing:" + url); } // Check Resource, not required yet a
                         * complete Resource Server because one credential has one resource else if
                         * (credential != null && credential.getResource() != null &&
                         * !url.equals(credential.getResource())) { // There is no credential, then
                         * returns an error httpResponse.sendError(HttpServletResponse.SC_UNAUTHORIZED,
                         * "Invalid resource :" + url); }
                         * 
                         * /** For the admin case
                         */
                        /*
                         * if ((url.startsWith(ADMIN_URI) || url.startsWith(CREDENTIAL_URI)) &&
                         * !user.equals(name)) // CKECK {
                         * 
                         * httpResponse.sendError(HttpServletResponse.SC_UNAUTHORIZED,
