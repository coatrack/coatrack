...
public class MetricsAggregationCustomRepositoryTest {
    ...
    /*
     * @Test public void getServiceApisByMetric() { List<Long> ids =
     * repository.getIDsOfLatestMetricsPerSession(null, null, true, true,
     * sampleDataApiProviderUsername); List<ServiceApi> result =
     * repository.getServiceApisByMetric(ids.get(0)); assertNotNull(result); }
     * 
     * @Test public void getIDsOfLatestMetricsPerSessionTrueTrue() { List<Long>
     * result = repository.getIDsOfLatestMetricsPerSession(null, null, true, true,
     * sampleDataApiProviderUsername); assertThat(result.size()).isEqualTo(9); }
     * 
     * @Test public void getIDsOfLatestMetricsPerSessionFindByProxyId() { List<Long>
     * resultList = repository.getIDsOfLatestMetricsPerSession(null, null, false,
     * true, sampleDataApiProviderUsername); System.out.println(resultList); Metric
     * resultItem = metricRepository.findOne(resultList.get(0));
     * 
     * resultList =
     * repository.getIDsOfLatestMetricsPerSession(resultItem.getProxy().getId(),
     * null, false, true, sampleDataApiProviderUsername); boolean found = false; for
     * (Long id : resultList) { Metric item = metricRepository.findOne(id); if
     * (resultItem.getProxy().getId().equals(item.getProxy().getId())) { found =
     * true; } } assertTrue(found); }
     * 
     * @Test public void getIDsOfLatestMetricsPerSessionFindByType() { List<Long>
     * resultList = repository.getIDsOfLatestMetricsPerSession(null, null, false,
     * true, sampleDataApiProviderUsername); System.out.println(resultList); Metric
     * resultItem = metricRepository.findOne(resultList.get(0));
     * 
     * resultList = repository.getIDsOfLatestMetricsPerSession(null,
     * resultItem.getType(), false, true, sampleDataApiProviderUsername); boolean
     * found = false; for (Long id : resultList) { Metric item =
     * metricRepository.findOne(id); if
     * (resultItem.getType().equals(item.getType())) { found = true; } }
     * assertTrue(found); }
     * 
     * @Test public void getIDsOfLatestMetricsPerSessionTrueFalse() { List<Long>
     * result = repository.getIDsOfLatestMetricsPerSession(null, null, true, false,
     * sampleDataApiProviderUsername); assertThat(result.size()).isEqualTo(9); }
     * 
     * @Test public void getIDsOfLatestMetricsPerSessionFalseFalse() { List<Long>
     * result = repository.getIDsOfLatestMetricsPerSession(null, null, false, false,
     * sampleDataApiProviderUsername); assertThat(result.size()).isEqualTo(41); }
     * 
     * @Test public void getSummarizedMetricsByProxyId() { List<Long> idResult =
     * repository.getIDsOfLatestMetricsPerSession(null, null, false, false,
     * sampleDataApiProviderUsername); Metric resultItem =
     * metricRepository.findOne(idResult.get(0));
     * 
     * List<MetricsAggregation> result =
     * repository.getSummarizedMetricsByProxyId(resultItem.getProxy().getId());
     * assertNotNull(result); for (MetricsAggregation item : result) {
     * System.out.println("::." + item.getServiceApiName() + " " + item.getApiUser()
     * + " " + item.getHttpResponseCode() + " " + item.getCount()); }
     * 
     * }
     */
    ...
    // In the following there are tests for each statistic displayed on the GUIs
    // main dashboard
    // when logging in as the user configured as "sampleDataApiProviderUsername",
    // which is the api provider username in the sample dataset
