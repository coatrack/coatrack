...
public class AdminServicesController {
    ...
    // TODO the following method puts the error message into the form, however it is never displayed to the user, as
    // this code is called via AJAX
    // so we should either replace AJAX with normal form submit OR send these error messages as AJAX response rather
    // than via ModelAndView
    private ModelAndView updateFormWithErrorMessage(ServiceApi serviceStored, String errorMessageKey,
    ...
    public String get(@PathVariable("id") long id, Model model,
        ...
        List<MetricsAggregation> metricsForThisService = metricsAggregationRepo
                .getSummarizedMetricsByServiceIdAndDateRange(id, session.getDashboardDateRangeStart(),
    ...
    @ResponseBody
    public Map<String, Map<String, Map<String, Map<String, Long>>>> get(@PathVariable("id") long id,
    ...
    public Map<String, DoughnutChart> generateUsageStatisticsDoughnutCharts(@PathVariable("id") long id) {
        ...
        Color[] doughnutChartColors = { Color.LIGHT_BLUE, Color.LIGHT_GRAY, Color.LIGHT_SALMON, Color.RED, Color.AZURE,
                Color.BLACK, Color.GREEN, Color.GREEN_YELLOW, Color.DARK_OLIVE_GREEN };
        ...
        buildMapOfStatisticsPerApiConsumer(metricsForThisService).forEach((consumer, statsPerMethodMap) -> {
            statsPerMethodMap.forEach((method, statsPerResponseCodeMap) -> {
                DoughnutDataset dataset = new DoughnutDataset().setLabel("HTTP response codes")
                        .addBackgroundColors(doughnutChartColors).setBorderWidth(2);
        ...
        buildMapOfStatisticsPerResponseCode(metricsForThisService).forEach((responseCode, statsPerMethodMap) -> {
            statsPerMethodMap.forEach((method, callsPerPathMap) -> {
                DoughnutDataset dataset = new DoughnutDataset().setLabel("Path")
                        .addBackgroundColors(doughnutChartColors).setBorderWidth(2);
                DoughnutData data = new DoughnutData().addDataset(dataset);
