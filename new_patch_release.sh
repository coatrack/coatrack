...
# Get previous patch version and increment with one
release_patch_version=$((patch+1))
echo "#################################################################################"
echo "Make sure you are on the release branch you want to create a patch for"
...
then
    ...
    # Build repository with new release_verion e.g. 12.0.1 if major was 12 and minor was 0 and patch was 1
    bash build.sh $release_version

    # Add,commit changes with release version as commit message
    git add .
    git commit -m "prepare release $release_version"
    # Create new tag for new patch release
    git tag -a $release_version -m "$release_version"
    git push origin $release_version
    # Change pom version to match new release
    mvn versions:set -DnewVersion=$release_version
    # Add and push pom changes to the major release branch
    # NOTE: creating a patch release will not create a new branch it will update the current release branch pom version
    git add .
    git commit -m "store pom patch release version changes"
